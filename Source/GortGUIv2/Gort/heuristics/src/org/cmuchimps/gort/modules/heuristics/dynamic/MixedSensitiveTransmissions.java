 ...
 * @author shahriyar
 */
public class MixedSensitiveTransmissions extends AbstractDynamicHeuristic {
    public static final String NAME = "Sensitive Transmissions w/ and w/o SSL";
    public static final String SUMMARY = "App transmits sensitive data both with and without SSL.";
    
    private boolean hasSSLTransmission, hasNonSSLTransmission;
    
    public MixedSensitiveTransmissions() {
        super(NAME, SUMMARY);
    }

    public MixedSensitiveTransmissions(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new MixedSensitiveTransmissions(project, apk, traversal);
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (type.startsWith(TaintHelper.TYPE_OS_NETWORK.toLowerCase()) ||
                type.startsWith(TaintHelper.TYPE_LIBCORE_OS_SEND.toLowerCase())) {
            hasNonSSLTransmission = true;
            
            if (state != null) {
                getStateIds().add(state.getId());
            }
        } else if (type.startsWith(TaintHelper.TYPE_SSL_OUTPUT.toLowerCase())) {
            hasSSLTransmission = true;
    ...
    public Boolean output() {
        super.output();
        return hasPermission(AndroidPermissions.INTERNET) && hasNonSSLTransmission &&
                hasSSLTransmission;
    }
