 ...
 * @author shahriyar
 */
public class TransmitsLastLocation extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Last Location";
    public static final String SUMMARY = "App transmits user's last location (i.e., as obtained by location provider's last location).";
    
    private boolean output = false;
    
    public TransmitsLastLocation() {
        super(NAME, SUMMARY);
    }

    public TransmitsLastLocation(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsLastLocation(project, apk, traversal);
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_LOCATION_LAST)) {
            output = true;
