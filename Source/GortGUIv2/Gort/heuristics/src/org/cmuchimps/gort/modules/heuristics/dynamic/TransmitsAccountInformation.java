 ...
 * @author shahriyar
 */
public class TransmitsAccountInformation extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Account Information";
    public static final String SUMMARY = "App transmits user's account information.";
    
    private boolean output = false;
    
    public TransmitsAccountInformation() {
        super(NAME, SUMMARY);
    }

    public TransmitsAccountInformation(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsAccountInformation(project, apk, traversal);
    }

    @Override
    public void init() {
        if (!hasPermission(AndroidPermissions.INTERNET) ||
                !hasAnyPermission(new String[]{AndroidPermissions.GET_ACCOUNTS,
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_ACCOUNT)) {
            output = true;
