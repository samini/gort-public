 ...
 * @author shahriyar
 */
public class TransmitsPhoneNumber extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Phone Number";
    public static final String SUMMARY = "App transmits user's phone number.";
    
    private boolean output = false;
    
    public TransmitsPhoneNumber() {
        super(NAME, SUMMARY);
    }

    public TransmitsPhoneNumber(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsPhoneNumber(project, apk, traversal);
    }

    @Override
    public void init() {
        if (!hasPermission(AndroidPermissions.INTERNET) ||
                !hasAnyPermission(new String[]{AndroidPermissions.READ_PHONE_STATE, AndroidPermissions.GET_ACCOUNTS})) {
            setComputed(true);
        }
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_PHONE_NUMBER)) {
            output = true;
