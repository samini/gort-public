 ...
 * @author shahriyar
 */
public class TransmitsExactLocation extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Exact Location";
    public static final String SUMMARY = "App transmits user's exact location (e.g., as obtained through GPS).";
    
    private boolean output = false;
    
    public TransmitsExactLocation() {
        super(NAME, SUMMARY);
    }

    public TransmitsExactLocation(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsExactLocation(project, apk, traversal);
    }

    @Override
    public void init() {
        if (!this.hasAllPermissions(new String[] {AndroidPermissions.INTERNET, AndroidPermissions.ACCESS_FINE_LOCATION})) {
            setComputed(true);
        }
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_LOCATION_GPS)) {
            output = true;
