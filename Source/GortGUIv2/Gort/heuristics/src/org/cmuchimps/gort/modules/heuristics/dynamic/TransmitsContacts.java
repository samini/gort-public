 ...
 * @author shahriyar
 */
public class TransmitsContacts extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Contacts";
    public static final String SUMMARY = "App transmits user's contacts.";
    
    private boolean output = false;
    
    public TransmitsContacts() {
        super(NAME, SUMMARY);
    }

    public TransmitsContacts(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsContacts(project, apk, traversal);
    }

    @Override
    public void init() {
        if (!this.hasAllPermissions(new String[] {AndroidPermissions.INTERNET, AndroidPermissions.READ_CONTACTS})) {
            setComputed(true);
        }
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_CONTACTS)) {
            output = true;
