 ...
 * @author shahriyar
 */
public class TransmitsApproximateLocation extends AbstractDynamicHeuristic {
    public static final String NAME = "Transmits User's Approximate Location";
    public static final String SUMMARY = "App transmits user's approxmiate location (e.g., as obtained through the Network).";
    
    private boolean output = false;
    
    public TransmitsApproximateLocation() {
        super(NAME, SUMMARY);
    }

    public TransmitsApproximateLocation(Project project, FileObject apk, Traversal traversal) {
        this();
    ...
    public AbstractDynamicHeuristic getInstance(Project project, FileObject apk, Traversal traversal) {
        return new TransmitsApproximateLocation(project, apk, traversal);
    }
    ...
    public void onTransmissionTaintLog(TaintLog t) {
        ...
        if (TaintHelper.checkTag(taintTag, TaintHelper.TAINT_LOCATION_NET)) {
            output = true;
